...
const sample_lab_info_model=require('../../models/echarts/update.js')
...
class echartCompute_Controller{
  ...
  static async ComputeWaterHistogramElementValue(ctx){
    let UpdateOptions = ctx.request.body //从前端传入的数据
    let { assess_type,statistic_value } = UpdateOptions
    let res = await AllEchartDataModel.getWaterElementData(assess_type)//得到所有的数据
   ...
   for(let key in obj){
      ...
      let obj_max = ele_list.reduce((acu, cur)=>{
          for(let key2 in cur){
              ...
              acu[key2] = Number(acu[key2]).toFixed(4)
          }
          return acu
      })
    ...
    for(let o=0;o<arr_max.length;o++){
      ...
      let  item2=arr_average[o]
          ...
          if(statistic_value == 'max_value'){
            ...
            const arr5 = await sample_lab_info_model.UpdateWaterElementData(statistic_value,item['point_num'],assess_type,item)
           
          }else if(statistic_value == 'mean_value'){
            console.log('statistic_value2',statistic_value)
            const arr6 = await sample_lab_info_model.UpdateWaterElementData(statistic_value,item2['point_num'],assess_type,item2)
          }
  ...
  static async ComputeWaterRadarElementEachDepthValue(ctx){
    ...
    let res = await AllEchartDataModel.getWaterData(sample_depth,assess_type)//得到所有的数据
    assess_type=Number(assess_type)
   ...
   for(let key in obj){
    ...
    for(let o=0;o<arr_max.length;o++){
      ...
      if(statistic_value == 'max_value'){
        console.log('statistic_value',statistic_value)
        const arr5 = await sample_lab_info_model.UpdateType2WaterElementEachDepthData(statistic_value,item['point_num'],sample_depth,item)
      }else if(statistic_value == 'mean_value'){
        console.log('statistic_value2',statistic_value)
        const arr6 = await sample_lab_info_model.UpdateType2WaterElementEachDepthData(statistic_value,item2['point_num'],sample_depth,item2)
      }
