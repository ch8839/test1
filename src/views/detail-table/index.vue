<template>
  <div class="project-info-container">
    <div class="project-selector">
      <el-cascader :options="ProjectOptions" expand-trigger="hover" v-model="selectedProject" @change="projectChange" />
    </div>
    ...
    <div class="content">
      <div class="content-head">
        ...
        <el-select v-model="selectedAssessType" placeholder="请选择调查类型" @change="assessTypeChange">
          <el-option v-for="item in assess_type_options" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </div>

      <el-tabs v-model="activeName" @tab-click="switchTab">
        <el-tab-pane label="土壤信息" name="first">
          <el-table v-loading="listLoading" :data="lab_ground_list" border stripe style="width: 100%">
            <el-table-column type="expand">
              <template slot-scope="props">
                <!-- <div v-if="props.row.labGround_element[0].ispollution==3" class="nodata">今日数据尚未更新</div> -->
                <div class="drop-container">
                  <div class="table2-container">
                    <el-table :data="props.row.labGround_element" :row-class-name="tableRowClassName"
                      class="drop_table">
                      <el-table-column label="指标">
                        <template slot-scope="scope">
                          <span :class="{ pollution: scope.row.ispollution }">
                            {{ element_Map.get(scope.row.element) }}
                          </span>
                        </template>
                      </el-table-column>
                      <el-table-column label="数值">
                        <template slot-scope="scope">
                          <span :class="{ pollution: scope.row.ispollution }">
                            {{ handleElementValue(scope.row.value) }}
                          </span>
                        </template>
                      </el-table-column>
            ...
            <el-table-column prop="point_num" label="调查类型" align="center">
              <template slot-scope="scope">
                <span>
                  {{ assess_type_options[scope.row.assess_type].label }}
                </span>
              </template>
            </el-table-column>
            <el-table-column label="位置" align="center">
              <template slot-scope="scope">
                <el-popover placement="right" width="400" v-if="scope.row.lng" trigger="click">
                  <div class="map_container2">
                    <el-amap ref="map2" :vid="scope.row.sample_num" :center="[scope.row.lng, scope.row.lat]"
                      :zoom="zoom" class="amap-demo">
                      <el-amap-marker :position="[scope.row.lng, scope.row.lat]" :vid="scope.row.id"
            ...
            <el-table-column label="污染物" align="center">
              <template slot-scope="scope">
                ...
                <template v-if="handlePollutionList(scope.row.labGround_element).length">
                  <el-tag v-for="(item, index) in handlePollutionList(
                      scope.row.labGround_element
                    )" type="danger" :key="index">{{ element_Map.get(item.element) }}</el-tag>
                </template>
                <template v-else>
          ...
          <div class="pagination">
            <el-pagination @size-change="handleSizeChange" :current-page="currentPage"
              @current-change="handleCurrentChange" :page-sizes="[2, 5, 8, 10]" :page-size="page_size"
              layout="total, sizes, prev, pager, next, jumper" background :total="total" />
          </div>
        </el-tab-pane>

        <el-tab-pane label="水质信息" name="second">
          <el-table v-loading="listLoading2" :data="lab_water_list" border stripe style="width: 100%">
            <el-table-column type="expand">
              <template slot-scope="props">
                <div class="drop-container">
                  <div class="table2-container">
                    <el-table :data="props.row.labWater_element" :row-class-name="tableRowClassName" class="drop_table">
                      <el-table-column label="指标">
                        <template slot-scope="scope">
                          <span :class="{ pollution: scope.row.ispollution }">
                            {{ element_Map.get(scope.row.element) }}
        ...
        <el-tab-pane label="监测点位分布" name="third">
          <div class="map_container">
            <el-amap ref="map" :center="center" vid="amapDemo" :zoom="zoom" :plugin="plugin" :events="events"
              class="amap-demo">
              <el-amap-marker v-for="(marker, index) in markers" :position="marker.position" :visible="marker.visible"
                :vid="index" animation="AMAP_ANIMATION_DROP" :key="index" />
