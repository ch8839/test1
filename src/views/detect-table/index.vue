<template>
  <div class="project-info-container2">
    <div class="project-selector">
      <el-cascader expand-trigger="hover" :options="ProjectOptions" v-model="selectedProject" @change="projectChange">
      </el-cascader>
    </div>
    ...
    <div class="content">
      ...
      <el-tabs v-model="activeName" @tab-click="switchTab">
        <el-tab-pane label="土壤信息" name="first">
          <el-table v-loading="listLoading" :data="det_ground_list" border stripe style="width: 100%"
            @expand-change="expandChange">
            <el-table-column type="expand">
              <template slot-scope="scope">
                <div class="drop-container">
                  <div class="table2-container">
                    <el-table :data="det_ground_reference" class="drop_table" :row-class-name="tableRowClassName">
                  ...
                  <div class="chart_contain">
                    <ve-histogram :data="handleChartData(det_ground_reference)" :settings="chartSettings3"
                      width="450px"></ve-histogram>
                  </div>
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="污染物" align="center">
              <template slot-scope="scope">
                ...
                <template v-if="handlePollutionList(scope.row.detGround_element).length">
                  <el-tag type="danger" v-for="(item,index) in handlePollutionList(scope.row.detGround_element)"
                    :key="index">{{element_Map.get(item.element)}}</el-tag>
                </template>
                <template v-else>
          ...
          <div class="pagination">
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
              :current-page="currentPage" :page-sizes="[2, 5, 8, 10]" :page-size="page_size"
              layout="total, sizes, prev, pager, next, jumper" background :total="total"></el-pagination>
          </div>
        </el-tab-pane>


        <el-tab-pane label="水质信息" name="second">
          <el-table v-loading="listLoading2" :data="det_water_list" border stripe style="width: 100%"
            @expand-change="expandChange2">
            <el-table-column type="expand">
              <template>
                <div class="drop-container">
                  <div class="table2-container">
                    <el-table :data="det_water_reference" class="drop_table" :row-class-name="tableRowClassName">
                      <el-table-column label="指标">
                        <template slot-scope="scope">
                          <span>{{ element_Map.get(scope.row.element) }}</span>
                        </template>
                  ...
                  <div class="chart_contain">
                    <ve-histogram :data="handleChartData(det_water_reference)" :settings="chartSettings3" width="450px">
                    </ve-histogram>
                  </div>
                </div>
              </template>
            ...
            <el-table-column label="污染物" align="center">
              <template slot-scope="scope">
                ...
                <template v-if="handlePollutionList(scope.row.detWater_element).length">
                  <el-tag type="danger" v-for="(item,index) in handlePollutionList(scope.row.detWater_element)"
                    :key="index">{{element_Map.get(item.element)}}</el-tag>
                </template>
                <template v-else>
                  <el-tag type="success">无污染元素</el-tag>
          ...
          <div class="map_container">
            <el-amap ref="map" vid="amapDemo" :center="center" :zoom="zoom" :plugin="plugin" :events="events"
              ...
              <el-amap-marker v-for="(marker, index) in markers" :position="marker.position" :visible="marker.visible"
                animation="AMAP_ANIMATION_DROP" :vid="index" :key="index"></el-amap-marker>
